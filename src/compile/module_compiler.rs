...
impl<'ctx, 'b, M: ModuleProvider<'ctx>> ModuleCompiler<'ctx, 'b, M> {
    ...
    pub fn decompose(self) -> (M, LLVMContext<'ctx, 'b>, SymbolTable) {
        (self.module_provider, self.context, self.symbols)
    }
}
impl<'ctx, 'b, M: ModuleProvider<'ctx>> ASTVisitor for ModuleCompiler<'ctx, 'b, M> {
    ...
    fn check_fn_declaration(&mut self, fn_declaration: &FnDeclaration) {
        ...
        self.scope_manager.insert(fn_declaration.get_name().get_index(), fn_ref.clone());
        ...
        for (ast_param, ir_param) in fn_declaration.get_args().iter().zip(fn_ref.get_params()) {
